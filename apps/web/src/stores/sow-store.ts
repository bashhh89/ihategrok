...
const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'https://demo.qandu.me';
...
export const useSOWStore = create<SOWStore>((set, get) => ({
  ...
  sendMessage: async (message: string, mode: 'plan' | 'build' = 'plan') => {
    ...
    // Check if this is a slash command for refinement (only specific commands)
    const validSlashCommands = ['/newScope', '/addRole', '/setBudget'];
    const isSlashCommand = validSlashCommands.some(cmd => message.trim().startsWith(cmd)) && !isGenerationCommand;
    ...
    try {
      if (isGenerationCommand) {
        ...
        // Regular conversation - bulletproof for demo
        try {
          ...
          if (!response.ok) {
            ...
            const mockReply: Message = {
              ...
              role: 'assistant',
              content: "I'm experiencing some technical difficulties with the AI service right now. However, you can still create and edit SOWs manually, and export them to PDF and Excel.",
              sowId: activeSow.id,
              createdAt: new Date(),
            };
            ...
            set((state) => ({
              activeSow: state.activeSow
                ? {
                    ...state.activeSow,
                    messages: [...state.activeSow.messages.filter(m => !m.id.startsWith('temp-')), mockReply],
                  }
                : null,
            }));
            return;
          ...
          const fallbackReply: Message = {
            id: `ai-demo-${Date.now()}`,
            role: 'assistant',
            content: "Sorry, I'm having trouble connecting to the AI service. You can still work with your SOWs manually - create, edit, and export to PDF/Excel are all working.",
            sowId: activeSow.id,
            createdAt: new Date(),
          };
          ...
          set((state) => ({
            activeSow: state.activeSow
              ? {
                  ...state.activeSow,
                  messages: [...state.activeSow.messages.filter(m => !m.id.startsWith('temp-')), fallbackReply],
                }
              : null,
          }));
        }
      ...
      // Honest error handling - never crash
      const safeErrorMessage: Message = {
        id: `demo-safe-${Math.random()}`,
        role: 'assistant',
        content: `I'm having technical issues with the AI features right now. The core SOW editor, PDF export, and Excel export are working fine though.`,
        sowId: activeSow.id,
        createdAt: new Date(),
      };
      ...
      set((state) => ({
        activeSow: state.activeSow
          ? {
              ...state.activeSow,
              messages: [...state.activeSow.messages.filter(m => !m.id.startsWith('temp-')), safeErrorMessage],
            }
          : null,
        errorMessage: null, // Clear error message for demo
      }));
    } finally {
      set({ isLoading: false });
    }
