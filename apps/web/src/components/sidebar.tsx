...
export function Sidebar() {
  ...
  const logoSrc = !mounted ? 'https://i.ibb.co/4gtm4Xm0/Logo-Dark-Green.png' : (document.documentElement.classList.contains('dark') ? '/footer-logo.png' : 'https://i.ibb.co/4gtm4Xm0/Logo-Dark-Green.png');
    ...
    <div className="w-64 border-r bg-card">
      <div className="p-4 border-b space-y-4">
        <div className="flex justify-center">
          <button
            onClick={() => {
              // If in settings or rate-card, go back. Otherwise go home
              if (pathname?.includes('/settings') || pathname?.includes('/rate-card')) {
                router.back();
              } else {
                router.push('/landing');
              }
        ...
        <Button
          onClick={async () => {
            if (activeSow && confirm('Are you sure you want to create a new SOW? Make sure your current work is saved.')) {
