...
function SortableRole({ id, children }: { id: number; children: React.ReactNode }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="border rounded p-3 mb-3">
      {children}
    </div>
  );
}
...
export function ProjectScopes() {
  ...
  // For new model: deliverables is a single HTML string
  const updateDeliverables = (scopeIndex: number, html: string) => {
    updateScope(scopeIndex, 'deliverables', html);
  };

  const addAssumption = (scopeIndex: number) => {
    const scope = sowData.scopes[scopeIndex];
    const updatedAssumptions = [...scope.assumptions, 'New Assumption'];
    updateScope(scopeIndex, 'assumptions', updatedAssumptions);
  };

  const updateAssumption = (scopeIndex: number, assumptionIndex: number, value: string) => {
    const scope = sowData.scopes[scopeIndex];
    const updatedAssumptions = [...scope.assumptions];
    updatedAssumptions[assumptionIndex] = value;
    updateScope(scopeIndex, 'assumptions', updatedAssumptions);
  };

  const deleteAssumption = (scopeIndex: number, assumptionIndex: number) => {
    const scope = sowData.scopes[scopeIndex];
    const updatedAssumptions = scope.assumptions.filter((_, i) => i !== assumptionIndex);
    updateScope(scopeIndex, 'assumptions', updatedAssumptions);
  };
  ...
  const handleDragEnd = (scopeIndex: number) => (event: DragEndEvent) => {
    const { active, over } = event;

    if (!over || active.id === over.id) return;

    const scope = sowData.scopes[scopeIndex];
    const oldIndex = scope.roles.findIndex((_, i) => i === active.id);
    const newIndex = scope.roles.findIndex((_, i) => i === over.id);

    const reorderedRoles = arrayMove(scope.roles, oldIndex, newIndex);
    updateScope(scopeIndex, 'roles', reorderedRoles);
  };
      ...
      {sowData.scopes.map((scope: Scope, scopeIndex: number) => (
        <Card key={scopeIndex}>
          ...
          <CardContent className="space-y-6">
            ...
            {/* Deliverables - single rich text editor */}
            <div>
              <Label>Deliverables</Label>
              <RichTextEditor
                content={Array.isArray(scope.deliverables) ? scope.deliverables.join('<br>') : (scope.deliverables || '')}
                onChange={(html) => updateDeliverables(scopeIndex, html)}
                placeholder="List all deliverables for this scope..."
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Button size="sm" onClick={() => addAssumption(scopeIndex)}>
                  <Plus className="h-3 w-3 mr-1" />
                  Add
                </Button>
              </div>
              {scope.assumptions.map((assumption, assumptionIndex) => (
                <div key={assumptionIndex} className="flex gap-2 mb-2">
                  <Input
                    value={assumption}
                    onChange={(e) => updateAssumption(scopeIndex, assumptionIndex, e.target.value)}
                    placeholder="Project assumption..."
                  />
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => deleteAssumption(scopeIndex, assumptionIndex)}
            ...
            <div>
              ...
              <DndContext
                ...
                onDragEnd={handleDragEnd(scopeIndex)}
              >
                <SortableContext
                  items={scope.roles.map((_, i) => i)}
                  strategy={verticalListSortingStrategy}
                >
              {scope.roles.map((role, roleIndex) => (
                <SortableRole key={roleIndex} id={roleIndex}>
                  <div className="flex gap-2 items-start">
                    <GripVertical className="h-5 w-5 text-muted-foreground cursor-grab active:cursor-grabbing mt-2 flex-shrink-0" />
