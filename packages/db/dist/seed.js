...
const rateCardData = [
    ...
    { name: "Account Management - (Offshore)", rate: 120 },
...
async function main() {
    console.log('Start seeding rate card with Social Garden Professional Rates ...');
    // Clear existing data first
    await prisma.rateCardItem.deleteMany();
    console.log('Cleared existing rate card items');
    for (const item of rateCardData) {
        const rateCardItem = await prisma.rateCardItem.create({
            data: item,
        });
        console.log(`✓ Created: ${rateCardItem.name} - $${rateCardItem.rate}/hr`);
    }
    console.log(`🎯 Seeding finished. Created ${rateCardData.length} professional rate card items.`);
}
main()
    .catch((e) => {
    console.error(e);
